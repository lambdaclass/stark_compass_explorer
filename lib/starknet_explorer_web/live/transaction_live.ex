          ...
          <%= for input <- @transaction.input_data do %>
            <%= unless is_nil(input.call) do %>
              <div class="w-full bg-black/20 p-5 mt-5">
                call <span class="text-se-violet"><%= input.call.name %></span>(<.intersperse :let={arg} enum={input.call.args}><:separator>, </:separator><span class="text-blue-400"><%= arg.name %></span></.intersperse>)
